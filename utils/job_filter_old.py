...
class JobFilter:
    ...
    def load_offers(self, filename="job_description.json"):
        ...
        with open(filename, "r", encoding="utf-8") as f:
            ...
            #self.job_data = job[self.]["description"]
    #print(job_data)
    def deadline_filter(self):
        """Filter job offers by deadline."""
        # today
        #today = datetime.today()
        print("job data", self.job_data)
        # deadline from job_description.json
        #deadline = self.job_data['deadline']
        #if deadline is a week before today
        if deadline == today - timedelta(days=7):
            return [job for job in self.job_data if job['deadline'] and job['deadline'] <= deadline]
        #        return [job for job in self.job_data if job['deadline'] and job['deadline'] <= deadline]
        elif deadline == today - timedelta(days=14):
            return [job for job in self.job_data if job['deadline'] and job['deadline'] <= deadline]
        else:  
            return [job for job in self.job_data]
...
if __name__ == "__main__":
    ...
    JobFilter.deadline_filter(datetime.today())